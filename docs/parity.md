...
This file tracks SmartCLI against the upstream Java, TypeScript, and Python implementations.
...
- MCP:
  ...
  - `smartcli mcp init-chrome`
  - `smartcli mcp list`
  - SmartCLI MCP server over stdio/http for built-in tools
- Chrome DevTools MCP:
...
uv run smartcli mcp serve --transport http --port 3999
```
