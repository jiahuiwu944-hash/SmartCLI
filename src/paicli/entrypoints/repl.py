...
async def _handle_slash(
    ...
    if command == "/help":
        ...
        context = agent.context_runtime.snapshot()
        table = Table(title="SmartCLI Context")
        table.add_column("Field")
        table.add_column("Value")
        table.add_row("cwd", cwd)
        table.add_row("model", f"{config.llm.model} ({config.llm.provider})")
        table.add_row("context window", f"{context.context_window:,} tokens")
        table.add_row("estimated input", f"{context.estimated_input_tokens:,} tokens")
        table.add_row("actual input", f"{context.actual_input_tokens:,} tokens")
        table.add_row("output reserve", f"{context.output_reserve:,} tokens")
        table.add_row("tool reserve", f"{context.tool_reserve:,} tokens")
        table.add_row("safety margin", f"{context.safety_margin:,} tokens")
        table.add_row("pressure", f"{context.pressure_ratio:.1%}")
        table.add_row("pressure level", context.pressure_level)
        table.add_row("compression target", f"{context.target_ratio:.1%}")
        table.add_row("calibration", f"{context.calibration_factor:.2f}x")
        table.add_row("compressions", str(context.compression_count))
        table.add_row("slimmed tool results", str(context.compacted_tool_results))
        table.add_row("summarized messages", str(context.summarized_messages))
        ...
        console.print(
            "Use `smartcli mcp serve --transport stdio|http --port 3000` to expose tools."
        )
    elif command == "/task":
